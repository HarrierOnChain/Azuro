...
> Автоматический **торговый бот Azuro** — Децентрализованный протокол. Часть набора [Prediction Market Toolkits](https://github.com/HarrierOnChain/Prediction-Markets-Trading-Bot-Toolkits): одно ядро исполнения, один слой риска, все площадки.

**Azuro** уже **работает в продакшене** сегодня.

---

## Azuro в реальной торговле
...
<sub>Общий TUI в работе на Azuro — позиции в реальном времени, P&L и состояние предохранителя. <!-- TODO: swap in Azuro-specific captures --></sub>
...
Эти боты работают на Azuro через один адаптер площадки на общем движке — те же риск-контроли, тот же TUI, полная поддержка dry-run.
...
> Нужна стратегия, не указанная здесь для Azuro? Покрытие адаптеров определяется спросом — [напишите](https://t.me/HarrierOnChain).
...
Этот репозиторий — точка входа для **Azuro**. Ядро исполнения, слой риска и все 20+ адаптеров площадок находятся в основном наборе:
...
> Торговля на рынках предсказаний сопряжена с реальным финансовым риском. ПО предоставляется «как есть», без гарантий, и не является финансовой консультацией. Всегда тестируйте с `enable_trading: false` перед вводом реального капитала. Соблюдайте условия использования Azuro и местное законодательство.

---

<div align="center">

**Торговый бот Azuro · построен на движке [Prediction Market Toolkits](https://github.com/HarrierOnChain/Prediction-Markets-Trading-Bot-Toolkits)**
